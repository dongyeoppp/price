...
class Character(Sprite):
    ...
    def movingball(self):
        if self.move1 == True:
            ...
            if self.y > 400 :             # 공의 y좌표값이 400이상일때 공이 다시 떨어지도록 함
                    self.y_velocity = -self.y_velocity 
                    self.x_velocity = -self.x_velocity 
            if self.x > 500 or self.x < - 500 : # 공의 x좌표값이 500이상이 되거나, -500이하로 되지않도록 함(벽에 튕겨나오는 것을 구현)
                    self.x_velocity = -self.x_velocity 
                    self.y_velocity = -self.y_velocity 
    def images1(self):  # images1,2는 캐릭터가 x좌표축으로 움직이도록 함 (velocity값으로 속도를 조절, if 문을 사용해 지정된 범위 내에서 움직이도록함 )
...
ball = Character(0,-300, 32, 32, "ball.gif", move1 = False)    # 기존의 좌표값을 변경. move1 을 False 값으로 두어 space를 눌렀을 때 공이 움직이기 시작하게 함)
...
def move_ball3():
    ball.move1 = True          # moveball3는 ball.move1 = True값으로 줌
...
while True:
    ...
    if ball.is_aabb_collision(goblin):
        goblin.image = "x.gif"                            # 공에 캐릭터가 충돌했을 경우 x이미지로 변경/ is_aabb_collision 충돌로 통일 사용
        
    if bar.is_aabb_collision(ball):
        ball.y_velocity =  -ball.y_velocity              # bar와 공이 충돌했을 경우에는 튕겨져 나갈 수 있도록 수정
    ball.movingball()   # space를 누른후 공의 움직임이 반복될 수 있도록 movingball함수를 while문에 넣어둠 , render로 도장 찍기
    ball.render(pen)
    ...
    pen.goto(500, -800)  
    pen.goto(500, 800)  
    pen.goto(-500, 800)  # 다시 첫 번째 꼭지점으로 돌아와 사각형을 완성, 사각형 틀로 맵의 크기를 눈으로 볼 수 있도록 표시
    pen.penup()             